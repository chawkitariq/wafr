...
/**
 * Central composable for the simulator page.
 *
 * Responsibilities:
 * - Holds the three reactive inputs (monthlyDeposit, initialCapital, years)
 * - Syncs those inputs to/from URL query params so simulations are shareable
 * - Exposes computed `results` derived from the pure `calculateResults` util
 * - Handles the share-URL copy flow
 *
 * URL param mapping (kept short for shareable links):
 *   s = monthly deposit (épargne mensuelle)
 *   c = initial capital
 *   y = years (années)
 */
export const useSimulator = () => {
  ...
  onMounted(() => {
    ...
    // Only apply if values are within allowed bounds — guards against malformed URLs
  ...
  // Sync params to URL so the current simulation is always bookmarkable/shareable
  ...
  const copyLink = async () => {
    // Clipboard API is browser-only; guard prevents accidental SSR execution
